...
- `canvas-core` 已导出统一会话存储适配接口，Web 与 Desktop 不再各写一套令牌存储逻辑。
...
- 运行时配置来源优先级已经冻结为 `__SDKWORK_CANVAS_ENV__ > import.meta.env > process.env`，页面层不再被允许自行拼装环境变量。
...
| `VITE_SDKWORK_CANVAS_APPLICATION_PUBLIC_HTTP_URL` | Notes App API 基地址（topology profile） |
| `VITE_SDKWORK_CANVAS_PLATFORM_API_GATEWAY_HTTP_URL` | 平台 / IAM API 网关基地址 |
...
- SDK client 配置源并非只来自 `import.meta.env`，而是按 `process.env -> import.meta.env -> __SDKWORK_CANVAS_ENV__` 合并，并以 `__SDKWORK_CANVAS_ENV__ > import.meta.env > process.env` 作为最终优先级口径。
...
- `pnpm --filter @sdkwork/canvas-pc-shell test`
- `pnpm --filter @sdkwork/canvas-pc-desktop test`
