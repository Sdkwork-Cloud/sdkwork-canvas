...
### 1. `canvas-canvas` 已把 note 元数据移动链路接入 `canvas-sync`

- `moveNote()` 成功后，现在会把父文件夹变更结果映射为一条 `note / move / queued` 同步任务。
- 这意味着 `canvas-sync` 不再只承接创建、保存、删除和恢复场景，而是已经开始进入关键元数据变更场景。
...
- 重新核对 `deleteNotePermanently` 后确认：远端语义是 `permanentlyDelete`，但当前 `canvas-sync` operation 集没有独立 `permanent-delete`。
...
- 下一步要么扩展 `canvas-sync` task model，要么将永久删除明确沉淀为后续 backlog，再进入 `CP08-4`。
