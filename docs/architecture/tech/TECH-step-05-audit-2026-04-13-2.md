...
3. `sdkwork-canvas-pc-react/packages/sdkwork-canvas-canvas/package.json`
   - `canvas-canvas` 正式声明对 `@sdkwork/canvas-observability` 的工作区依赖。
4. `sdkwork-canvas-pc-react/tsconfig.base.json`
   - 新增 `@sdkwork/canvas-observability` 的 workspace source alias，修复 `tsc` 解析边界。
...
1. 本轮先出现过一次真实失败：`@sdkwork/canvas-observability` 未进入 `tsconfig.base.json` 路径映射，导致 `tsc` 无法解析；问题已修复并重新验证。
...
3. 保存链最小 telemetry sink 接口已经建立，后续可由 `@sdkwork/canvas-observability` 提供真实实现。
