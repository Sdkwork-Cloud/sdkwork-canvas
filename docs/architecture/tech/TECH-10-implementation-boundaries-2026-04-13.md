...
- canvas 仓库的认证宿主边界重新冻结为 `@sdkwork/canvas-auth`

## 2. 代码与合同落地

- 更新 `sdkwork-canvas-pc-react/scripts/workspace-boundary-contract.test.mjs`
  - 将 shared auth 的边界校验收口到 `packages/sdkwork-canvas-auth`
  - 冻结 `sdkworkAuthBridge.ts` 对 `canvas-core` 的 runtime 绑定
  - 冻结除 `canvas-auth` 外其它 TypeScript 包不得直连 `@sdkwork/auth-pc-react`
