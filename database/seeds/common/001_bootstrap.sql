-- SDKWork Canvas bootstrap 种子数据(跨语言通用)
-- 注入系统默认设置,提供三层配置模型的顶层默认值。
-- tenant_id = '0' 表示系统级(非租户专属)。

-- 系统设置:外观默认值
INSERT INTO cvs_system_setting (id, namespace, setting_key, setting_value, value_type, scope, scope_value, created_by, updated_by)
VALUES
