...
pub(crate) const TRAY_MENU_ID_OPEN_CANVAS: &str = "open_canvas";
...
pub(crate) fn tray_action_for_menu_id(id: &str) -> Option<TrayAction> {
    match id {
        ...
        TRAY_MENU_ID_OPEN_CANVAS => Some(TrayAction::OpenRoute(ROUTE_CANVAS)),
...
pub(crate) fn build_tray_menu_spec(language: TrayLanguage) -> Vec<TrayMenuEntry> {
    ...
    vec![
        ...
        TrayMenuEntry::Submenu {
            ...
            items: vec![
                TrayMenuEntry::Item {
                    id: TRAY_MENU_ID_OPEN_CANVAS,
                    label: labels\.canvas.to_string(),
                },
                TrayMenuEntry::Item {
...
fn open_route_from_tray<R: Runtime>(app: &AppHandle<R>, route: &str) -> DesktopResult<()> {
    ...
    if let Some(window) = app.get_webview_window(MAIN_WINDOW_LABEL) {
        ...
        let script = format!(
            "window.__canvas_PENDING_TRAY_ROUTE__ = {route}; window.dispatchEvent(new CustomEvent('canvas:tray-navigate', {{ detail: {{ route: {route} }} }}));",
            route = route_literal
        );
...
fn build_tray_menu<R: Runtime>(
    ...
    let navigate_menu = SubmenuBuilder::new(app, labels.navigate)
        .text(TRAY_MENU_ID_OPEN_CANVAS, labels.canvas)
...
mod tests {
    ...
    fn tray_menu_ids_map_to_expected_actions() {
        ...
        assert_eq!(
            tray_action_for_menu_id("open_canvas"),
            Some(TrayAction::OpenRoute("/canvas"))
        );
        assert_eq!(
