...
- 新增 `packages/sdkwork-canvas-canvas/src/services/noteWorkspaceAutosave.ts`
- 新增 `createNotesWorkspaceAutosavePlan()`
- 新增 `CANVAS_WORKSPACE_AUTOSAVE_DELAY_MS`
...
- `sdkwork-canvas-pc-react/package.json` 的 `test:workspace:contracts` 已纳入 autosave contract
- `sdkwork-canvas-pc-react/scripts/package-scripts-contract.test.mjs` 与脚本聚合保持一致
