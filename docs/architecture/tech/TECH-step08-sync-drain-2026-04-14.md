...
- `canvas-sync` 的 browser queue store 已支持订阅最新 queue snapshot。
- `canvas-canvas` 工作区 store 已新增 `syncQueueSnapshot` 与 `requestSyncDrain()`。
- 工作区 selector / presentation model / insights panel 已串起同步卡片：
  - 状态
  - 徽章
  - 明细
  - 可选手动 drain 入口
- 国际化资源已补齐 `canvas.sync.*` 与 `canvas.actions.retrySync`。
