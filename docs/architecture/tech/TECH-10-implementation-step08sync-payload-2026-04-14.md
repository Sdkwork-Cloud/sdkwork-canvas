...
- `canvas-canvas` 已在所有已接入的 note 主写入路径中写入显式 `mutation`。
- worker、worker runtime、workspace runtime boundary 的现有合同已适配新任务结构并继续通过。

## 当前架构事实

1. `@sdkwork/canvas-sync` 现在冻结的是“带可执行意图的任务模型”，不是只有 operation 名称的元数据记录。
2. queue schema 1 被视为 legacy envelope，会在读取时直接降级为空队列。
3. 该降级策略在当前 direct-write 前提下是安全的，因为旧任务并非未发送的本地权威写入。
4. `canvas-canvas` 当前只负责产出正确 `mutation`，仍不默认创建真实 replay handler。

## 影响范围

- `packages/sdkwork-canvas-sync`
- `packages/sdkwork-canvas-canvas`
