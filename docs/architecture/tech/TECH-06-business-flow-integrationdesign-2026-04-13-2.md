...
1. `canvas.workspace.save.retry.scheduled`
   - 级别：`warn`
   - 语义：一次失败后的自动重试已被安排
2. `canvas.workspace.save.retry.recovered`
   - 级别：`info`
   - 语义：经过自动重试后保存恢复成功
3. `canvas.workspace.save.retry.exhausted`
   - 级别：`error`
   - 语义：自动退避次数已耗尽，进入终态失败

接口边界说明：

1. `canvas-canvas` 只依赖 `@sdkwork/canvas-observability` 的类型。
2. 真实 sink 实现仍由后续观测能力 step 提供。
3. `tsconfig.base.json` 已增加 `@sdkwork/canvas-observability` workspace alias，确保当前工作区 `tsc` 可解析。
