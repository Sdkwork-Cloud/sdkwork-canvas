...
const baselinePath = path.join(
  root,
  'database/ddl/baseline/postgres/0001_canvas_baseline.sql',
);
...
const prefixRegistry = {
  ...
  prefixes: [{ prefix: 'canvas_', owner: 'canvas-platform', domain: 'canvas' }],
};
...
const schemaYaml = [
  ...
  'owner_team: canvas-platform',
  'compliance_level: L2',
  'engines:',
  '  - postgres',
  'table_prefix: canvas_',
  'tables:',
  ...tableNames.map(
    (name) => `  - name: ${name}\n    lifecycle_status: active\n    owner: canvas-platform`,
  ),
...
process.stdout.write(`materialized ${tableNames.length} tables into Canvas Database contract\n`);
