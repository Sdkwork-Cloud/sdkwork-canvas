...
async function createBundleContext(options) {
  ...
  const stageName = `sdkwork-canvas-api-gateway-config-${version}`;
...
async function bundleCloudConfig(context) {
  ...
  for (const configName of CANVAS_CLOUD_GATEWAY_CONFIGS) {
...
Included configs:
${CANVAS_CLOUD_GATEWAY_CONFIGS.map((name) => `- configs/${name}`).join('\n')}
`;
