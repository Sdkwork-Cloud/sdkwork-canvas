...
test('bundle plan resolves canvas desktop cwd and target environment metadata', () => {
  const plan = createDesktopReleaseBuildPlan({
    cwd: path.join('D:', 'workspace', 'sdkwork-canvas-pc-react'),
  ...
  assert.equal(
    plan.cwd,
    path.join('D:', 'workspace', 'sdkwork-canvas-pc-react', 'packages', 'sdkwork-canvas-pc-desktop'),
  );
  ...
  assert.equal(plan.workspaceRoot, path.join('D:', 'workspace', 'sdkwork-canvas-pc-react'));
  assert.equal(plan.phase, 'bundle');
});

test('bundle plan constrains macOS bundles to app output', () => {
  const plan = createDesktopReleaseBuildPlan({
    cwd: '/workspace/sdkwork-canvas-pc-react',
