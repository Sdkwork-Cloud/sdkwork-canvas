...
1. 新增 `sdkwork-canvas-pc-react/packages/sdkwork-canvas-canvas/src/components/NotesWorkspaceDialogFooter.tsx`，建立对话框底部适配边界。
2. `sdkwork-canvas-pc-react/packages/sdkwork-canvas-canvas/src/pages/NotesWorkspacePage.tsx` 删除页面内联的 `footer={(<>...</>)}` 与 `Button` 装配，改为直接消费 `NotesWorkspaceDialogFooter`。
3. `sdkwork-canvas-pc-react/packages/sdkwork-canvas-canvas/src/components/index.ts` 已导出新组件，确保边界具备稳定入口。
4. 新增 `sdkwork-canvas-pc-react/scripts/workspace-page-dialog-footer-boundary.contract.test.mjs`，冻结“对话框底部最终 UI 绑定不得回退到页面层”的 source contract。
