...
3. `canvas-search` 已成为本轮工作区搜索接线的唯一事实来源。
...
- `sdkwork-canvas-pc-react/scripts/workspace-command-palette.contract.test.mjs`
...
- `sdkwork-canvas-pc-react/packages/sdkwork-canvas-canvas/src/services/noteWorkspaceCommandPaletteModel.ts`

本轮实现将 `canvas-search` 接入到两个既有 service model：
...
以上改动确保 `canvas-canvas` 能在当前 monorepo 下稳定解析并消费 `@sdkwork/canvas-search`。
