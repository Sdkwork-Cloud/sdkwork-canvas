...
1. `sdkwork-canvas-pc-react/packages/sdkwork-canvas-canvas/src/services/noteWorkspaceAutosave.ts`
...
2. `sdkwork-canvas-pc-react/packages/sdkwork-canvas-canvas/src/pages/NotesWorkspacePage.tsx`
...
3. `sdkwork-canvas-pc-react/scripts/workspace-autosave.contract.test.mjs`
   - 新增 `shouldFlush` 断言。
   - 冻结 `saveState === 'error'` 的正确计划结果：允许 flush，不允许重新调度延迟 autosave。
4. `sdkwork-canvas-pc-react/scripts/workspace-save-flush-boundary.contract.test.mjs`
   - 新增统一刷盘入口 contract。
   - 冻结“手动保存、快捷键保存、页面隐藏刷盘必须共享 `flushDraft`”这一架构事实。
5. `sdkwork-canvas-pc-react/package.json`
   - 将 `workspace-save-flush-boundary.contract.test.mjs` 纳入 `test:workspace:contracts`。
6. `sdkwork-canvas-pc-react/scripts/package-scripts-contract.test.mjs`
   - 同步冻结新的 contract 聚合链。
