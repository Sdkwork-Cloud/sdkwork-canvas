...
- `sdkwork-canvas-pc-react/scripts/workspace-search-schema.contract.test.mjs`

该 contract 通过真实的 `@sdkwork/canvas-search` 源码入口，冻结以下事实：
...
- `sdkwork-canvas-pc-react/packages/sdkwork-canvas-search/src/index.ts`

本轮在 `canvas-search` 内冻结了最小但完整的统一检索模型：
...
1. `canvas-search` 已不再只是占位接口，当前已具备稳定的统一 schema 边界。
