> Migrated from `docs/release/Step04-创建笔记流程收敛-2026-04-07.md` on 2026-06-24.
...
本次增量将笔记工作区 create note 的页面级流程从 `NotesWorkspacePage.tsx` 中抽离到独立 runtime service，并将对应 contract 纳入 `test:workspace:contracts` 聚合链。
...
- `packages/sdkwork-canvas-canvas/src/services/index.ts`
...
- 风险说明: `hotkey runtime`、`sidebar resize runtime` 以及 store 内剩余写路径协同仍未完全抽离
