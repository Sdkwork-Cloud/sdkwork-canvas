...
3. 调用 `planRenamedFolderState()` 拼装 folders、canvas、trashedNotes、activeNote、selectedFolderId、expandedFolderIds。
...
1. 接收当前 folders、canvas、trashedNotes、activeNote、expandedFolderIds、selectedFolderId 的快照。
...
3. coordinator 的引入不能破坏现有 `pnpm.cmd --filter @sdkwork/canvas-canvas typecheck` 与 `pnpm.cmd typecheck`。
