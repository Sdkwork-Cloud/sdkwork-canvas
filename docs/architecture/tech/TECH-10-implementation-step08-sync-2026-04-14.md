...
1. `sdkwork-canvas-pc-react/packages/sdkwork-canvas-canvas/src/services/noteWorkspaceSelectors.ts`
   - `syncSummary` 现在新增：
     - `primaryEntityId`
     - `primaryMessage`
2. `sdkwork-canvas-pc-react/packages/sdkwork-canvas-canvas/src/services/noteWorkspacePagePresentationModel.ts`
   - `syncCard` 现在新增：
     - `actionKind`
     - `actionTargetNoteId`
   - 同步卡片已形成两类动作语义：
     - `review-note`
     - `retry-sync`
3. `sdkwork-canvas-pc-react/packages/sdkwork-canvas-canvas/src/pages/NotesWorkspacePage.tsx`
   - 页面容器已按 `actionKind` 分流到：
     - `requestSyncDrain()`
     - `selectNote(noteId)`
4. 国际化资源已补齐：
   - `canvas.actions.reviewSyncIssue`
5. 合同测试已补齐并冻结：
