...
- `canvas-canvas` 已新增 `createNotesWorkspaceSyncRuntime(...)` 作为 workspace-side runtime 边界。
...
- `canvas-sync` 已新增 `NotesSyncRemoteApplyRequest`，把未来远端执行输入冻结为显式 request。
...
- `canvas-canvas` 的 `createNotesWorkspaceSyncRuntime(...)` 现在已支持 `apply(request)` 注入。
- `bootstrapNotesWorkspaceStore(...)` 现在也支持 `apply(request)` 注入，并能据此创建 workspace sync runtime。
- `canvas-shell` 与 `canvas-desktop` 当前已形成单一顶层注入路径：
...
- `contracts/canvas-remote-apply-app-sdk-target.contract.json` 当前已冻结 future semantic target contract：
...
## 2026-04-14 / CP08-4 / canvas-canvas 工作区 store bootstrap 装配边界
...
- `sdkwork-canvas-pc-react/packages/sdkwork-canvas-shell/src/application/providers/AppProviders.tsx`
  - `AppProviders` 现在会真实接收并透传 `canvasWorkspaceBootstrapOptions`，不再只是声明可选 props。
- `sdkwork-canvas-pc-react/scripts/workspace-store-bootstrap.contract.test.mjs`
  - 已修正旧的 `bootstrapNotesWorkspaceStore()` 断言口径，改为显式校验 `bootstrapNotesWorkspaceStore(canvasWorkspaceBootstrapOptions ?? {})`。
...
- `sdkwork-canvas-pc-react/contracts/canvas-remote-apply-app-sdk-service.contract.json`
  - 已冻结 future `@sdkwork/canvas-core` shared-wrapper public service surface：
...
- `sdkwork-canvas-pc-react/scripts/workspace-sync-app-sdk-service-contract.test.mjs`
  - 已把 service spec、target contract、result adapter contract 与当前 canvas-core service pattern 绑定为根级 guardrail。
...
- 当前 `@sdkwork/canvas-core` 仍没有真实 `appNoteSyncService` 实现。
...
- 当前 `@sdkwork/canvas-core` 仍不能进入真实 `appNoteSyncService` 实现阶段。
...
- 当前 `@sdkwork/canvas-core` 仍不能进入真实 shared-wrapper 实现阶段。
...
- `sdkwork-canvas-pc-react/contracts/canvas-remote-apply-app-sdk-result-adapter.contract.json`
  - 已冻结 shared wrapper owner：`@sdkwork/canvas-core`
...
- 当前 `@sdkwork/canvas-core` 仍没有真实 result adapter 实现。
...
- `sdkwork-canvas-pc-react/scripts/workspace-sync-app-sdk-target-contract.test.mjs`
  ...
  - 已校验该 spec 与 `packages/sdkwork-canvas-sync/src/index.ts` 当前 `NotesSyncRemoteApplyRequest` 字段映射一致。
- `sdkwork-canvas-pc-react/package.json`
- `sdkwork-canvas-pc-react/scripts/package-scripts-contract.test.mjs`
...
- `sdkwork-canvas-pc-react/packages/sdkwork-canvas-i18n/src/resources/en-US.ts`
- `sdkwork-canvas-pc-react/packages/sdkwork-canvas-i18n/src/resources/zh-CN.ts`
  - 已新增 `canvas.actions.reviewSyncIssue`。
