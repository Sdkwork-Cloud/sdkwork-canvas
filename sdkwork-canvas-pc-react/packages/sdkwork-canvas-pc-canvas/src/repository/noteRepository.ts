...
class AppSdkNoteRepository implements NoteRepository {
  ...
  constructor(options: AppSdkNoteRepositoryOptions = {}) {
    const defaultWorkspaceReadStrategy = options.workspaceReadStrategy ?? createWorkspaceSnapshotReadStrategy({
      listActiveNoteSummaries: async () => {
        const canvas = await this.listActiveNoteVOs();
        return canvas.map((item) => mapNoteSummary(item));
      },
      listDeletedNoteSummaries: async (keyword) => {
        const canvas = await this.listDeletedNoteVOs(keyword);
        return canvas.map((item) => mapNoteSummary(item));
      },
  ...
  private async listNotesPage(params: QueryParams) {
    ...
    return unwrapRequiredData<PageNoteVO>(response, 'Failed to list canvas');
  }
  ...
  async findAll(pageRequest?: PageRequest): Promise<ServiceResult<Page<NoteSummary>>> {
    try {
      ...
      return Result.error(toErrorMessage(error, 'Failed to query canvas'));
    }
  }
  ...
  async findTrashed(pageRequest?: PageRequest): Promise<ServiceResult<Page<NoteSummary>>> {
    try {
      ...
      return Result.error(toErrorMessage(error, 'Failed to query trashed canvas'));
    }
  }
  ...
  async deleteAll(ids: string[]): Promise<ServiceResult<void>> {
    for (const id of ids) {
      ...
      if (!result.success) {
        return Result.error(result.message || 'Failed to delete canvas');
      }
    }
    return Result.success(undefined);
  }
  ...
  async saveAll(entities: Partial<NoteSummary>[]): Promise<ServiceResult<NoteSummary[]>> {
    ...
    for (const entity of entities) {
      ...
      if (!result.success || !result.data) {
        return Result.error(result.message || 'Failed to save canvas');
      }
