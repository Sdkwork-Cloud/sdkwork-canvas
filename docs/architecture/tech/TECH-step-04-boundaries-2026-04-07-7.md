...
1. 新增 `sdkwork-canvas-pc-react/packages/sdkwork-canvas-canvas/src/components/NotesWorkspaceErrorBanner.tsx`，建立错误提示适配边界。
2. `sdkwork-canvas-pc-react/packages/sdkwork-canvas-canvas/src/pages/NotesWorkspacePage.tsx` 删除页面内的 `errorMessage ? (...) : null`，改为直接消费 `NotesWorkspaceErrorBanner`。
3. `sdkwork-canvas-pc-react/packages/sdkwork-canvas-canvas/src/components/index.ts` 已导出新组件，确保边界具备稳定入口。
4. 新增 `sdkwork-canvas-pc-react/scripts/workspace-page-error-banner-boundary.contract.test.mjs`，冻结“错误提示最终 UI 绑定不得回退到页面层”的 source contract。
